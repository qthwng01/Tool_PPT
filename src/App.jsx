...
const App = () => {
  ...
  const filterExcel = (name) => {
    ...
    const arrs = excels.filter(x => x["Tên Đại Lý"].toLowerCase() === checkName)
    //console.log(arrs)
    ...
    if (city === "" || region === "") {
      ...
      if (folders.length === 0) {
        ...
        folders.forEach((folder) => {
          ...
          slide.addText(`${region} – ${city} – ${infoOnExcel.qh} - ĐL ` + folder.name, {
            x: 1,
            y: 0.5,
            w: "90%",
            h: "10%",
    ...
    <div className="tools">
      ...
      <div className="info">
        ...
        <div className="tools_inside">
          ...
          <Button
            isLoading={isLoading}
            onClick={createPPT} colorScheme="teal" size="lg">
            Tạo .PPTX
          </Button>
          {/* <button onClick={createPPT}>Tạo .PPTX</button> */}
        </div>
      </div>
    </div>
  );
